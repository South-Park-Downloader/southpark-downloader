...
export default class Filter {
  ...
  constructor(input: string) {
    ...
    console.debug(
      `Initializing new Filter with left "${left}" and right "${right}"`
    );
    ...
    if (right) {
      ...
      if (!this.hasEpisode(left)) {
        ...
        this.right = new Stop(
          `S${this.left.season}E${Number.MAX_SAFE_INTEGER}`
        );
      }
    }
    ...
    console.log(
      `Initialized Filter (${
        this.isRange() ? 'range' : 'specific'
      }) with left: '${this.left.format()}'${
        this.right ? ` and right: '${this.right.format()}'` : '.'
      }`
    );
  }
  ...
  public match(episode: EpisodeDatum): boolean {
    ...
    if (this.isRange()) {
      ...
      return (
        ...
        (typeof this.left.episode !== 'undefined'
          ? episode.index === this.left.episode
          : true)
      );
    }
  }
  ...
  private checkRightBounds(episode: EpisodeDatum): boolean {
    ...
    return (
      ...
      /* verify episode within episode bounds if episode season is same as season bounds */
      (episode.season === this.right.season &&
      typeof this.right.episode !== 'undefined'
        ? episode.index <= this.right.episode
        : true)
    );
  }
