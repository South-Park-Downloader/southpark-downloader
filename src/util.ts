...
export async function readdirRecursive(path: PathLike, options?: ReaddirOptions): Promise<string[]> {
  ...
  const children = (await readdir(path, options)).map(child => resolve(path.toString(), child));

  /* Get directories from children */
  const childDirectories = await filter(children, async child => (await lstat(child)).isDirectory());

  /* Parallel recurse into directories and flatten results */
  const results = (await Promise.all(childDirectories.map(childDirectory => readdirRecursive(childDirectory)))).flat();
...
export async function filter<T>(array: T[], callback: (element: T) => Promise<boolean>): Promise<T[]> {
  /* Use new symbol as error value so we can use array with any value */
  const failed = Symbol();

  /* Create promises for all elements with the given callback */
  const promises = array.map(async element => await callback(element) ? element : failed);
